...
public class AnalyzersCompositeBuilder {
	...
	public Composite getTabContainer(CTabFolder folder, String consoleConfig) {
		if (consoleConfigTab.containsKey(consoleConfig)) {
			return consoleConfigTab.get(consoleConfig);
		}
		Composite newTab = createTab(folder, consoleConfig);
		consoleConfigTab.put(consoleConfig, newTab);
		return newTab;
	}
	
	protected Composite createTab(CTabFolder folder, final String consoleConfig) {
		Composite container = new Composite(folder, SWT.TOP);
		container.setLayout(new GridLayout(2, true));
		
		createToolbar(container, consoleConfig);
		...
		input.addModifyListener(new ModifyListener() {
			...
			public void modifyText(ModifyEvent e) {
				IHSearchService service = HSearchServiceLookup.findService(HSearchConsoleConfigurationPreferences.getHSearchVersion(consoleConfig));
